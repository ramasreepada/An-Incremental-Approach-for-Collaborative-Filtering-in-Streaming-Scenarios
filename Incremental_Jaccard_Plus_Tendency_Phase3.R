...
incremental_train_set = rbind(incremental_train_set, train_set_part3)
...
{
  ratings_UI_test[train_set_part4[i,1], train_set_part4[i,2]] = train_set_part4[i,3]
...
results = matrix(0, nrow = nrow(train_set_part4), ncol = 2)

for(i in 1:as.numeric(nrow(train_set_part4)))
{
  user_i = train_set_part4[i,1]
  item_i = train_set_part4[i,2]
  expected_rating = train_set_part4[i,3]
...
mae_phase3 = mae/nrow(train_set_part4)

write.csv(results, 'Results_phase3.csv')

print('total time - phase 3')
print(total_time)
################################################################################
source('D:/Research_Recommender Systems/All codes/ECIR Incremental Updates 2018/Yahoo Music/Incremental_Jaccard_Plus_Tendency_Phase4.R')