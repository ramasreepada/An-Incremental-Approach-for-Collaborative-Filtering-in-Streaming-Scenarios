...
incremental_train_set = rbind(incremental_train_set, train_set_part2)
...
for(i in 1:nrow(train_set_part3))
{
  ratings_UI_test[train_set_part3[i,1], train_set_part3[i,2]] = train_set_part3[i,3]
...
############## TESTING PHASE  #######################################
######################################################################################
results = matrix(0, nrow = nrow(train_set_part3), ncol = 2)

for(i in 1:as.numeric(nrow(train_set_part3)))
{
  user_i = train_set_part3[i,1]
  item_i = train_set_part3[i,2]
  expected_rating = train_set_part3[i,3]
...
mae_phase2 = mae/nrow(train_set_part3)

write.csv(results, 'Results_phase2.csv')

print('total time - phase 2')
print(total_time)

source('D:/Research_Recommender Systems/All codes/ECIR Incremental Updates 2018/Yahoo Music/Incremental_Jaccard_Plus_Tendency_Phase3.R')
